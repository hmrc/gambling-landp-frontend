...
class InterestControllerITSpec extends AnyFreeSpec with Matchers with OptionValues with WireMockSupport with ScalaFutures with IntegrationPatience {
  ...
  private val url = routes.InterestController.onPageLoad(interestId).url
  ...
  "InterestController" - {
    ...
    "successful page load" - {
      ...
      "must return Not Found with page not found content when pageNo exceeds totalPages" in {
        ...
        running(app) {
          val request = FakeRequest(GET, routes.InterestController.onPageLoad(interestId, pageSize, 99).url)
            .withSession(SessionKeys.regime -> regime, SessionKeys.regNumber -> regNumber)
          val result = route(app, request).value
