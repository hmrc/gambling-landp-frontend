...
import models.interest.{InterestAccruingDetails, InterestAccruingDetailsItem, InterestDetails, InterestDetailsItem}
...
class GamblingConnectorSpec extends AnyFreeSpec with Matchers with WireMockSupport with ScalaFutures with IntegrationPatience {
  ...
  "GamblingConnector" - {
    ...
    "getInterestDetails" - {
      ...
      val expectedInterestResponse = InterestDetails(
        periodStartDate = Some(LocalDate.of(2024, 1, 1)),
        periodEndDate   = Some(LocalDate.of(2024, 12, 31)),
        total           = BigDecimal("123.45"),
        totalRecords    = 1,
      ...
      "must return a deserialized InterestDetails for a 200 response" in {
        stubFor(
          get(urlEqualTo(s"/gambling/interest-drilldown/$regime/$regNumber/$interestId?pageSize=$pageSize&pageNo=$pageNo"))
            .willReturn(okJson(interestResponseJson))
        )

        val app = buildApp()
        running(app) {
          val connector = app.injector.instanceOf[GamblingConnector]
          val result = connector.getInterestDetails(regime, regNumber, interestId, pageSize, pageNo).futureValue

          result mustEqual expectedInterestResponse
        }
      }
      ...
      "must forward the correct regime and registration number in the URL" in {
        ...
        running(app) {
          val connector = app.injector.instanceOf[GamblingConnector]
          val result = connector.getInterestDetails(otherRegime, otherRegNumber, interestId, pageSize, pageNo).futureValue
      ...
      "must forward the correct interestId in the URL" in {
        ...
        running(app) {
          val connector = app.injector.instanceOf[GamblingConnector]
          val result = connector.getInterestDetails(regime, regNumber, otherId, pageSize, pageNo).futureValue
      ...
      "must forward custom pageSize and pageNo query parameters" in {
        ...
        running(app) {
          val connector = app.injector.instanceOf[GamblingConnector]
          val result = connector.getInterestDetails(regime, regNumber, interestId, customPageSize, customPageNo).futureValue
