...
import models.interest.{InterestDetails, InterestDetailsItem}
...
class InterestControllerSpec extends SpecBase with MockitoSugar {
  ...
  val interestDetails: InterestDetails = InterestDetails(
  ...
  private val multiPageDetails = interestDetails.copy(totalRecords = 25)

  "InterestController" - {

    def url = routes.InterestController.onPageLoad("INT-001").url
    ...
    Seq(
      ...
      s"must render the heading and paragraph for description code $code ($label)" in {
        ...
        when(mockService.getInterest(any(), any(), any(), any(), any())(any()))
          .thenReturn(Future.successful(interestDetails.copy(descriptionCode = code)))
    ...
    "must not render pagination when there is only one page" in {
      val mockService = mock[GamblingService]
      when(mockService.getInterest(any(), any(), any(), any(), any())(any()))
        .thenReturn(Future.successful(interestDetails))
    ...
    "must redirect to page not found when data has 0 items" in {
      val mockService = mock[GamblingService]
      when(mockService.getInterest(any(), any(), any(), any(), any())(any()))
        .thenReturn(Future.successful(interestDetails.copy(items = Seq.empty, total = BigDecimal(0))))
