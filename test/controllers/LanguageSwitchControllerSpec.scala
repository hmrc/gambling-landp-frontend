...
class LanguageSwitchControllerSpec extends SpecBase {
  ...
  "LanguageSwitchController" - {
    ...
    "must redirect to the fallback URL when switching to English" in {
    ...
    "must redirect when switching to Welsh" in {
      val app = applicationBuilder().build()
    ...
    "must redirect to the fallback URL when no Referer header is present" in {
      val app = applicationBuilder().build()
