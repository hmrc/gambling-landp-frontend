...
class ReallocationsInControllerSpec extends SpecBase with MockitoSugar {
  ...
  private def url = routes.ReallocationsInController.onPageLoad().url
  ...
  "ReallocationsInController" - {
    ...
    "must redirect to the last page when pageNo exceeds totalPages" in {
      ...
      running(app) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.ReallocationsInController.onPageLoad(10, 3).url
      }
    }
