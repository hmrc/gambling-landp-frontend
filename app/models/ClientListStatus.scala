...
object ClientListStatus {
  ...
  val values: Seq[ClientListStatus] = Seq(InitiateDownload, InProgress, Succeeded, Failed)

  def fromString(status: String): Option[ClientListStatus] = values.find(_.asString == status)
}
